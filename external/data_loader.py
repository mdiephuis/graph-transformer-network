...
def load_local_data(data_path, name, one_hot=False, attributes=True, use_node_deg=False, wl=0):
    ...
    if name == 'IGN04':
        path = data_path + '/IGN/2004/'
        if attributes:
            dataset = build_IGN_dataset(path, type_attr='real', use_node_deg=use_node_deg)
        else:
            dataset = build_IGN_dataset(path)
    if name == 'IGN19':
        path = data_path + '/IGN/2019/'
        if attributes:
            dataset = build_IGN_dataset(path, type_attr='real', use_node_deg=use_node_deg, prefix='IGN19')
        else:
...
def histog(X, bins=10):
    ...
    return np.array(node_length), {'histo': np.histogram(np.array(node_length), bins=bins), 'med': np.median(np.array(node_length)), 'max': np.max(np.array(node_length)), 'min': np.min(np.array(node_length))}
    ...
    with open(path + name) as f:
        ...
        for elt in sections[0]:
            ...
            if np.isnan(node_dic[k]).any():  # then there are None values
                node_dic[k] = [0.00 if math.isnan(x) else x for x in node_dic[k]]  # remove NaNs
            else:
                node_dic[k] = [x for x in node_dic[k]]  # x/max(node_dic[k])normalize
...
def build_IGN_dataset(path, type_attr='label', use_node_deg=False, prefix='IGN04'):
    graphs = graph_label_list(path, prefix + '_graph_labels.txt')
    if type_attr == 'label':
        node_dic = node_labels_dic(path, prefix + '_node_labels.txt')  # A voir pour les attributes
    if type_attr == 'real':
        node_dic = node_attr_dic(path, prefix + '_node_attributes.txt')
    adjency = compute_adjency(path, prefix + '_A.txt')
    data_dict = graph_indicator(path, prefix + '_graph_indicator.txt')
    data = []
    for i in graphs:
        g = Graph()
        for node in data_dict[i[0]]:
